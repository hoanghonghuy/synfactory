...
func (a *CLIAdapter) Probe(ctx context.Context) error {
	...
	if len(a.probeArgs) == 0 {
		return nil
	}
	probeCtx, cancel := context.WithTimeout(ctx, a.probeLimit)
	defer cancel()
	cmd := exec.CommandContext(probeCtx, a.binary, a.probeArgs...)
	if err := cmd.Run(); err != nil {
		if errors.Is(probeCtx.Err(), context.DeadlineExceeded) {
			return Failure(FailureTransient, fmt.Errorf("probe %s timed out", a.name))
		}
...
func (a *CLIAdapter) execute(ctx context.Context, sessionID string, request Request) (Result, error) {
	...
	if err != nil {
		...
		if parseErr != nil {
			return result, errors.Join(err, parseErr)
		}
		return result, err
	}
	if parseErr != nil {
		result.Outcome = OutcomeFailed
		return result, Failure(FailurePermanent, parseErr)
	}
	if result.Outcome == "" {
...
func parseGenericJSON(process ProcessResult, runtimeName, model string) (Result, error) {
	...
	if err != nil {
		if strings.TrimSpace(process.Stdout) != "" {
			result.Summary = strings.TrimSpace(process.Stdout)
			return result, nil
		}
		return result, err
	}
	for _, object := range objects {
		...
		if session := findString(object, "conversation_id", "session_id", "thread_id", "sessionID", "id"); session != "" {
			result.SessionID = session
		}
		if summary := findString(object, "response", "result", "text", "output_text"); summary != "" {
			result.Summary = summary
		}
		if item, ok := object["item"].(map[string]any); ok {
			if summary := findString(item, "text", "content", "message"); summary != "" {
				result.Summary = summary
			}
		}
	}
...
func decodeJSONObjects(output string) ([]map[string]any, error) {
	...
	for _, line := range strings.Split(trimmed, "\n") {
		...
		if err := json.Unmarshal([]byte(line), &object); err != nil {
			return nil, fmt.Errorf("decode runtime JSON line: %w", err)
		}
		objects = append(objects, object)
	}
	if len(objects) == 0 {
		return nil, errors.New("runtime returned no JSON events")
	}
	return objects, nil
}
