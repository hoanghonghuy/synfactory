...
func TestVerifierUsesProcessExitStatusAndHashesEvidence(t *testing.T) {
	...
	if !report.Passed || report.SHA256 == "" || len(report.Checks) != 2 || !report.Checks[0].Passed || report.Checks[1].Passed { t.Fatalf("unexpected report %+v", report) }
}
...
func TestVerifierRequiredFailureFailsReport(t *testing.T) {
	...
	if err == nil || report.Passed { t.Fatalf("expected required verification failure") }
}
