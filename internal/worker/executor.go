...
func New(store Store, engine RuntimeEngine, builder RequestBuilder, cfg Config) *Worker {
	if cfg.ID == "" { cfg.ID = "worker" }
	if cfg.Capacity <= 0 { cfg.Capacity = 1 }
	if cfg.PollInterval <= 0 { cfg.PollInterval = 5 * time.Second }
	if cfg.LeaseDuration <= 0 { cfg.LeaseDuration = 2 * time.Minute }
	if cfg.HeartbeatInterval <= 0 { cfg.HeartbeatInterval = 30 * time.Second }
	if cfg.DefaultTimeout <= 0 { cfg.DefaultTimeout = 30 * time.Minute }
	if cfg.RetryBase <= 0 { cfg.RetryBase = 30 * time.Second }
...
func (w *Worker) Run(ctx context.Context) error {
	if w.store == nil || w.engine == nil || w.builder == nil { return errors.New("worker store, runtime engine and request builder are required") }
	go w.heartbeatLoop(ctx)
	for {
		worked, err := w.RunOne(ctx)
		if err != nil && !errors.Is(err, context.Canceled) { slog.Error("worker job failed", "worker", w.cfg.ID, "error", err) }
		if ctx.Err() != nil { return ctx.Err() }
		if worked { continue }
		timer := time.NewTimer(w.cfg.PollInterval)
		select {
		case <-ctx.Done(): if !timer.Stop() { <-timer.C }; return ctx.Err()
		case <-timer.C:
		}
	}
}
...
func (w *Worker) RunOne(ctx context.Context) (bool, error) {
	...
	if err != nil { return true, fmt.Errorf("start claimed job %s: %w", claimed.ID, err) }
	repository, err := w.store.GetRepository(ctx, job.RepositoryID)
	if err != nil { return true, w.failJob(ctx, job, fmt.Errorf("load repository: %w", err)) }
	request, err := w.builder.Build(ctx, job, repository)
	if err != nil { return true, w.failJob(ctx, job, fmt.Errorf("build runtime request: %w", err)) }
	request.RunID = job.ID
	request.Role = string(job.Role)
	request.Repository = repository.FullName
	if request.Timeout <= 0 { request.Timeout = w.cfg.DefaultTimeout }
	if request.Metadata == nil { request.Metadata = map[string]string{} }
	...
	if w.workspaces != nil {
		...
		if err != nil { return true, w.failJob(ctx, job, fmt.Errorf("acquire workspace: %w", err)) }
	...
	cancel()
	select { case err := <-leaseErr: if err != nil { runErr = errors.Join(runErr, fmt.Errorf("job lease lost: %w", err)) }; default: }
	if ctx.Err() != nil {
		if handle.Path != "" { _ = w.workspaces.Release(context.Background(), handle) }
		return true, ctx.Err()
	}

	if handle.Path != "" {
		if err := w.workspaces.Validate(ctx, handle); err != nil { runErr = errors.Join(runErr, err) }
	}
	if runErr == nil && result.Outcome == runtimefactory.OutcomeSucceeded && w.verifier != nil && w.planner != nil {
		...
		if planErr != nil {
			...
			if len(attempts) > 0 {
				...
				if evidenceErr != nil { runErr = errors.Join(runErr, fmt.Errorf("persist verification evidence: %w", evidenceErr)) }
			}
			if verifyErr != nil { runErr = errors.Join(runErr, verifyErr) }
		}
	}
	if handle.Path != "" {
		if err := w.workspaces.Release(ctx, handle); err != nil { runErr = errors.Join(runErr, fmt.Errorf("release workspace: %w", err)) }
	}
	if runErr == nil && result.Outcome == runtimefactory.OutcomeSucceeded {
		if _, err := w.store.SucceedJob(ctx, job.ID, w.cfg.ID, w.now()); err != nil { return true, fmt.Errorf("complete job %s: %w", job.ID, err) }
		return true, nil
	}
	if runErr == nil { runErr = fmt.Errorf("runtime ended with outcome %s", result.Outcome) }
	return true, w.failJob(ctx, job, runErr)
}

func workspaceMode(value string) workspace.Mode {
	if workspace.Mode(value) == workspace.ModeDocker { return workspace.ModeDocker }
...
func (w *Worker) keepLease(ctx context.Context, jobID string, cancel context.CancelFunc, result chan<- error) {
	interval := w.cfg.LeaseDuration / 3; if interval < time.Second { interval = time.Second }
	ticker := time.NewTicker(interval); defer ticker.Stop()
	for { select {
	case <-ctx.Done(): return
	case <-ticker.C:
		if _, err := w.store.RenewLease(ctx, jobID, w.cfg.ID, w.now(), w.cfg.LeaseDuration); err != nil { select { case result <- err: default: }; cancel(); return }
	} }
}
func (w *Worker) failJob(ctx context.Context, job domain.Job, runErr error) error {
	now := w.now(); retryAt := now.Add(retryDelay(w.cfg.RetryBase, job.Attempt))
	_, err := w.store.FailJob(ctx, job.ID, w.cfg.ID, now, retryAt, runErr.Error())
	if err != nil { return errors.Join(runErr, fmt.Errorf("persist job failure: %w", err)) }
	return runErr
}
func (w *Worker) heartbeatLoop(ctx context.Context) {
	worker := postgres.Worker{ID: w.cfg.ID, Host: w.cfg.Host, Capacity: w.cfg.Capacity}
	for {
		if _, err := w.store.HeartbeatWorker(ctx, worker, w.now()); err != nil && !errors.Is(err, context.Canceled) { slog.Error("worker heartbeat failed", "worker", w.cfg.ID, "error", err) }
		timer := time.NewTimer(w.cfg.HeartbeatInterval)
		select { case <-ctx.Done(): if !timer.Stop() { <-timer.C }; return; case <-timer.C: }
	}
}
func retryDelay(base time.Duration, attempt int) time.Duration {
	if base <= 0 { base = 30 * time.Second }; if attempt < 1 { attempt = 1 }; shift := attempt - 1; if shift > 6 { shift = 6 }; return base * time.Duration(1<<shift)
}

type runObserver struct { store Store; job domain.Job; now func() time.Time }
...
func (o *runObserver) AttemptFinished(ctx context.Context, attempt runtimefactory.Attempt) error {
	status := runStatus(attempt.Result.Outcome, attempt.Err); exitCode := attempt.Result.ExitCode; summary := attempt.Result.Summary
	if summary == "" && attempt.Err != nil { summary = attempt.Err.Error() }
	runID := runID(o.job.ID, o.job.Attempt, attempt.Sequence)
	if _, err := o.store.FinishRun(ctx, runID, status, o.now(), &exitCode, summary, attempt.Result.SessionID); err != nil { return err }
...
func runStatus(outcome runtimefactory.Outcome, err error) string {
	if err != nil { switch runtimefactory.ClassifyFailure(err) { case runtimefactory.FailureTimeout: return "timed_out"; case runtimefactory.FailureCanceled: return "cancelled"; default: return "failed" } }
	if outcome == runtimefactory.OutcomeSucceeded { return "succeeded" }; return "failed"
}
func runID(jobID string, attempt, sequence int) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%s\x00%d\x00%d", jobID, attempt, sequence))); return "run_" + hex.EncodeToString(sum[:12])
}
