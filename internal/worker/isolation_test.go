...
func (m *fakeWorkspaceManager) Release(context.Context, workspace.Handle) error {
	m.released = true
	return nil
}
...
func (p staticPlanner) Plan(context.Context, domain.Job, postgres.Repository, runtimefactory.Request) (verifier.Plan, error) {
	return p.plan, nil
}
...
func TestVerificationFailurePreventsJobSuccessAndReleasesWorkspace(t *testing.T) {
	...
	if !worked || err == nil {
		t.Fatalf("expected verification failure, worked=%v err=%v", worked, err)
	}
	if store.succeeded || !store.failed {
		t.Fatalf("verification must gate success: success=%v failed=%v", store.succeeded, store.failed)
	}
	if !manager.released {
		t.Fatal("workspace was not released")
	}
	if len(store.evidence) != 2 || store.evidence[1].Kind != "verification" || store.evidence[1].SHA256 == "" {
		t.Fatalf("verification evidence missing: %+v", store.evidence)
	}
}
