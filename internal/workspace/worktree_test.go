...
func TestReadOnlyWorktreeDetectsMutationAndCleansUp(t *testing.T) {
	...
	if err := manager.Validate(context.Background(), handle); !errors.Is(err, ErrUnauthorizedMutation) { t.Fatalf("expected mutation error, got %v", err) }
	if err := manager.Release(context.Background(), handle); err != nil { t.Fatal(err) }
	if _, err := os.Stat(handle.Path); !os.IsNotExist(err) { t.Fatalf("workspace still exists: %v", err) }
}
...
func run(t *testing.T, dir, name string, args ...string) {
	...
	cmd.Dir = dir
	if out, err := cmd.CombinedOutput(); err != nil { t.Fatalf("%s %v: %v: %s", name, args, err, out) }
}
